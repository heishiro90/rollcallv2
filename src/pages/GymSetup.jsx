...
export default function GymSetup() {
  ...
  async function createGym(e) {
    ...
    try {
      ...
      if (err) throw err;
      await supabase.from('gym_members').insert({ gym_id: gym.id, user_id: user.id, role: 'owner' });
      await refreshData();
  ...
  async function joinGym(e) {
    e.preventDefault(); setError(''); setLoading(true);
    try {
      const { data: gym } = await supabase.from('gyms').select('*').eq('invite_code', inviteCode.toUpperCase().trim()).single();
      if (!gym) throw new Error('Invalid invite code.');
      const { error: err } = await supabase.from('gym_members').insert({ gym_id: gym.id, user_id: user.id, role: 'member' });
    ...
    <div className="page" style={{ justifyContent: 'center', alignItems: 'center', padding: 20 }}>
      <div style={{ width: '100%', maxWidth: 380 }}>
        ...
        {!mode ? (
          ...
          <form onSubmit={joinGym} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div><label className="label">Invite Code</label><input className="input" placeholder="X7K2M9" value={inviteCode} onChange={e => setInviteCode(e.target.value)} style={{ textTransform: 'uppercase', letterSpacing: 4, textAlign: 'center', fontSize: 20, fontWeight: 700 }} required maxLength={8} /></div>
          ...
          <form onSubmit={createGym} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
