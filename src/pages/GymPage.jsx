...
// ─── Modal to add/edit manual member ───
function MemberModal({ member, gymId, userId, onSave, onClose }) {
  ...
  async function save() {
    ...
    try {
      if (member?.id) {
        await supabase.from('manual_gym_members').update({ name:name.trim(), belt, is_visitor:isVisitor, notes:notes.trim()||null }).eq('id', member.id);
      } else {
        await supabase.from('manual_gym_members').insert({ gym_id:gymId, created_by:userId, name:name.trim(), belt, is_visitor:isVisitor, notes:notes.trim()||null });
      }
      onSave();
      ...
      <div style={{ background:'#13131f', borderRadius:'16px 16px 0 0', width:'100%', maxWidth:480, padding:24, paddingBottom:40 }}>
        ...
        <div style={{ marginBottom:14 }}>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            {BELTS.map(b => (
              <button key={b} onClick={() => setBelt(b)} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:4, padding:'8px 10px', borderRadius:10, cursor:'pointer', border:belt===b?`1px solid ${BELT_COLORS[b]}`:'1px solid var(--border)', background:belt===b?`${BELT_COLORS[b]}15`:'transparent' }}>
        ...
        <div style={{ marginBottom:14, display:'flex', alignItems:'center', gap:10 }}>
          <button onClick={() => setIsVisitor(!isVisitor)} style={{ width:20, height:20, borderRadius:5, border:'1px solid var(--border)', background:isVisitor?'var(--accent)':'transparent', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
            {isVisitor && <span style={{ color:'#fff', fontSize:12 }}>✓</span>}
          </button>
        ...
        <div style={{ marginBottom:16 }}>
          ...
          <input className="input" placeholder="e.g. Training since 2022, comp record..." value={notes} onChange={e=>setNotes(e.target.value)} />
        </div>

        {err && <div style={{ color:'#ff6b6b', fontSize:12, marginBottom:8 }}>{err}</div>}
        <button className="btn btn-primary" onClick={save} disabled={busy} style={{ width:'100%' }}>{busy ? 'Saving...' : member?.id ? 'Save Changes' : 'Add Member'}</button>
      </div>
    </div>
  );
}
...
export default function GymPage() {
  ...
  const isOwner = members.some(m => m.user_id === user?.id) && gym?.owner_id === user?.id;
  ...
  return (
    <div className="container" style={{ paddingTop:24, paddingBottom:100 }}>
          ...
          {lb.map((e,i) => {
            ...
            return (
              <div key={e.user_id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', background:isMe?'rgba(123,45,142,.08)':'rgba(255,255,255,.015)', border:isMe?'1px solid rgba(123,45,142,.3)':'1px solid var(--border)', borderRadius:12 }}>
                <div style={{ width:24, textAlign:'center', fontSize:14, fontWeight:700, color:i===0?'var(--gold)':i===1?'#c0c0c0':i===2?'#cd7f32':'var(--text-muted)' }}>{i+1}</div>
                <Avatar entry={e} />
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:11, color:'var(--text-muted)', marginTop:2 }}>{e.gi_sessions} gi · {e.nogi_sessions} no-gi · {e.open_mat_sessions} open mat</div>
                </div>
      ...
      {/* Manual members section — always visible to owner, read-only to others */}
      <div style={{ marginBottom:16 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
          <button onClick={() => setShowManual(!showManual)} style={{ background:'none', border:'none', cursor:'pointer', display:'flex', alignItems:'center', gap:6 }}>
            <span style={{ fontSize:14, fontWeight:600, color:'#ccc' }}>Club Members</span>
            <span style={{ fontSize:11, color:'var(--text-muted)' }}>{manualMembers.length} added</span>
            <span style={{ color:'var(--text-muted)', fontSize:10 }}>{showManual?'▲':'▼'}</span>
          </button>
          {gym?.owner_id === user?.id && (
            <button onClick={() => { setEditMember(null); setShowMemberModal(true); }} style={{ padding:'6px 12px', borderRadius:8, background:'var(--accent)', border:'none', color:'#fff', fontSize:12, fontWeight:600, cursor:'pointer' }}>+ Add</button>
          )}
        </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            {manualMembers.length === 0 && (
              <div className="card" style={{ textAlign:'center', padding:24, color:'var(--text-muted)', fontSize:13 }}>
                No members added yet.{gym?.owner_id === user?.id && ' Use "+ Add" to add club members who don\'t use the app.'}
              </div>
            )}
            {manualMembers.map(m => (
              <div key={m.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 14px', background:'rgba(255,255,255,.02)', border:'1px solid var(--border)', borderRadius:12 }}>
                <Avatar entry={{ display_name:m.name, belt:m.belt, avatar_url:null }} size={32} />
                <div style={{ flex:1 }}>
                ...
                {gym?.owner_id === user?.id && (
                  <div style={{ display:'flex', gap:6 }}>
                    <button onClick={() => { setEditMember(m); setShowMemberModal(true); }} style={{ padding:'4px 8px', borderRadius:6, background:'rgba(255,255,255,.05)', border:'1px solid var(--border)', color:'var(--text-dim)', fontSize:11, cursor:'pointer' }}>Edit</button>
                    <button onClick={() => deleteMember(m.id)} style={{ padding:'4px 8px', borderRadius:6, background:'rgba(239,83,80,.08)', border:'1px solid rgba(239,83,80,.2)', color:'#ef5350', fontSize:11, cursor:'pointer' }}>✕</button>
                  </div>
                )}
              </div>
            ))}
